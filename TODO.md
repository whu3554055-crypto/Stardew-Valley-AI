...
- [ ] **B1 — AI 任务落地**  
  ...
  *进度：客户端 `AIQuestSystem` 的 AI 增强链路已从占位改为真实调用统一入口 `AIAgentManager.request_text_generation()`，并加入 JSON 校验与失败降级；已补最小目标验证闭环（`fetch` / `delivery` 按背包计数自动验证，`delivery` 消耗材料后完成）；后续继续补奖励写回一致性。*
