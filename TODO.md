...
- [ ] **A3 — 呈现与资源**  
  ...
  *进度：`TileMap` 已挂 `GameTileMap` + 运行时/资源图集 `assets/tiles/terrain_atlas_32.png`；玩家已用 `player.png`（缩放至约 32px）。田间作物按阶段 `Sprite2D`；背包格对有图标的物品显示缩略图 + `ItemDatabase.resolve_icon_path`。缺图标的条目仍用文字；可继续补全 `assets/sprites/items/**` 与种子图。*
