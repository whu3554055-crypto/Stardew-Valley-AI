...
## B6 + G2: mine shell with **same combat loop as `main`** (spawns, sword, kill rewards) + pickaxe gather.
...
func _ready() -> void:
	...
	if _player:
		...
		if _player.has_signal("attack_requested"):
...
func _maintain_combat_spawns() -> void:
	...
	if _was_in_mine_last_frame and not in_mine_now and (_run_kills > 0 or _run_bonus_gold > 0):
		...
		_mine_journal_event("Mine run recap: %d★, kills %d, elites %d, bonus +%dg, MVP %s." % [run_stars, _run_kills, _run_elites, _run_bonus_gold, mvp])
		_show_message("Run recap %d★ · MVP %s" % [run_stars, mvp], 1.2)
		_run_kills = 0
		_run_elites = 0
		_run_bonus_gold = 0
...
func _spawn_mine_enemy() -> bool:
	...
	if randf() < elite_chance or _no_elite_kill_streak >= ELITE_PITY_KILLS:
		...
		_mine_journal_event("An elite foe appears in the mine!")
		_show_message("Elite incoming!", 0.95)
		_play_screen_shake(4.2)
		if GatheringSfx:
...
func _on_player_attack_requested(origin: Vector2, facing: Vector2) -> void:
	...
	if GameManager and not GameManager.try_consume_stamina(ATTACK_STAMINA_COST):
		_show_message("Not enough stamina to attack.", 0.65)
		return
	...
	if now_sec > _combo_expire_at:
		if _combo_hits >= 2:
			_show_message("Combo dropped.", 0.35)
		_combo_hits = 0
	...
	for c in _enemy_layer.get_children():
		...
		if e.global_position.distance_to(center) <= range_v and facing_n.dot(to_enemy) >= ATTACK_CONE_DOT_MIN:
			...
			if not special_tag.is_empty():
				_show_message(special_tag, 0.45)
			elif dmg_to_apply >= BIG_HIT_THRESHOLD:
				_show_message("Heavy hit %d!" % dmg_to_apply, 0.4)
			if killed:
				...
				if overkill >= 10 and GameManager:
					...
					_show_message("Overkill +%dg" % overkill_bonus, 0.55)
					_mine_journal_event("Overkill payout: +%dg." % overkill_bonus)
	...
	if hit_any:
		...
		if _combo_hits >= COMBO_FEEDBACK_MIN_STACK:
			...
			_show_message(combo_label, 0.45)
			if _combo_hits == COMBO_MAX_STACKS:
				_mine_journal_event("Combo peak reached!")
		if GatheringSfx:
		...
		if is_crit:
			...
			_show_message("Critical hit!", 0.55)
			if _crit_chain >= 3 and GameManager:
				...
				_mine_journal_event("Critical chain x%d (+%dg)." % [_crit_chain, crit_bonus])
				_show_message("Critical chain x%d!" % _crit_chain, 0.8)
				_crit_chain = 0
		else:
			_crit_chain = 0
	else:
		if _combo_hits >= 2:
			_show_message("Combo missed.", 0.35)
		_combo_hits = 0
		_crit_chain = 0
...
func _on_enemy_contact_hit(_enemy: EnemyMelee, damage: float) -> void:
	...
	if _shield_charges > 0:
		...
		_show_message("Shield blocked the hit!", 0.45)
		return
	...
	if guarded and _enemy and _enemy.has_method("apply_knockback"):
		...
		_show_message("Perfect guard x%d" % _perfect_guard_chain, 0.45)
		if _perfect_guard_chain % 3 == 0 and GameManager:
			...
			_mine_journal_event("Perfect guard chain bonus +%dg." % guard_bonus)
	...
	if hp_cur_check - final_damage <= 0.0 and not _last_stand_used_run:
		...
		_mine_journal_event("Last Stand triggered! You survived with 1 HP.")
		_show_message("Last Stand!", 0.9)
		_mine_ui_refresh()
		return
	var alive: bool = GameManager.apply_damage(final_damage)
	_no_hit_kill_streak = 0
	_play_screen_shake(3.8 if panic_mode else 2.6)
	_revenge_buff_until = now + REVENGE_BUFF_SEC
	if guarded:
		_show_message("Guarded hit!", 0.4)
	elif panic_mode:
		_show_message("Panic guard activated!", 0.5)
	else:
		_show_message("You were hit!", 0.35)
	_mine_ui_refresh()
	if not alive:
...
func _handle_player_defeat() -> void:
	...
	if charged_gold:
		_show_message("You collapsed. Lost 60g.", 4.0)
	else:
		_show_message("You collapsed. Daily rescue covered the loss.", 4.0)
	_mine_ui_refresh()
	if _enemy_layer:
		for c in _enemy_layer.get_children():
			c.queue_free()
...
func _on_enemy_killed(enemy: EnemyMelee) -> void:
	...
	if _last_stand_redeem_until > 0.0 and now_sec <= _last_stand_redeem_until and GameManager:
		...
		_mine_journal_event("Redemption! Last Stand converted to victory (+%dg)." % redeem_gold)
		_show_message("Redemption +%dg" % redeem_gold, 1.0)
	...
	if GameManager:
		...
		if _kill_streak > best_streak:
			...
			_mine_journal_event("New personal best streak: %d." % _kill_streak)
			_show_message("New PB streak: %d" % _kill_streak, 0.85)
	...
	if hype_now != _hype_rank:
		_hype_rank = hype_now
		_show_message("Hype rank: %s" % _hype_rank, 0.85)
		_mine_journal_event("Combat hype advanced to %s." % _hype_rank)
	...
	if dropped_ore:
		...
		if _no_ore_kill_streak >= ORE_PITY_KILL_THRESHOLD:
			...
			if not pity_tpl.is_empty():
				...
				_no_ore_kill_streak = 0
				_show_message("Ore pity drop: %s" % pity_item_id, 0.9)
	if QuestSystem:
	...
	if _no_hit_kill_streak == NO_HIT_STREAK_GOAL and GameManager:
		...
		_mine_journal_event("No-hit streak achieved! +%dg." % no_hit_bonus)
		_show_message("No-hit streak x%d!" % NO_HIT_STREAK_GOAL, 1.0)
	...
	if _momentum_score >= MOMENTUM_STEP:
		...
		_show_message("Momentum surge! +%dg" % momentum_bonus, 0.8)
		_mine_journal_event("Momentum payout: +%dg." % momentum_bonus)
	if GameManager:
		var hp_cur: float = float(GameManager.player_data.get("hp", 100.0))
		var hp_max: float = maxf(1.0, float(GameManager.player_data.get("hp_max", 100.0)))
		if hp_cur / hp_max <= CLUTCH_HP_RATIO:
			GameManager.player_data["gold"] = int(GameManager.player_data.get("gold", 0)) + CLUTCH_BONUS_GOLD
			_mine_journal_event("Clutch kill! +%dg bonus." % CLUTCH_BONUS_GOLD)
			_show_message("Clutch kill!", 0.8)
	if GameManager:
		...
		if is_elite:
			...
			_show_message("Elite progress today: %d / 5" % mini(elite_today, 5), 0.9)
			if elite_today == 5:
				_mine_journal_event("Daily elite hunter target reached (5/5).")
				_show_message("Daily elite target complete!", 1.0)
			if not bool(GameManager.player_data.get("combat_badge_first_elite", false)):
				...
				_mine_journal_event("Badge unlocked: First Elite Down.")
				_show_message("Badge: First Elite Down", 1.2)
	if is_elite:
		_show_message("Elite defeated!", 0.8)
	else:
		_show_message("Enemy defeated.", 0.35)
	if GameManager and is_elite:
		...
		_mine_journal_event("Elite bounty claimed (+%dg)." % bounty_gold)
		_show_message("Elite bounty +%dg" % bounty_gold, 0.9)
		...
		if not bonus_item_id.is_empty():
			...
			if not bonus_tpl.is_empty():
				...
				_show_message("Elite bonus drop: %s" % bonus_item_id, 0.8)
	else:
	...
	if _enemy_layer:
		for c in _enemy_layer.get_children():
			...
			if near_enemy.global_position.distance_to(enemy.global_position) <= KILL_SPLASH_RANGE:
				...
				splash_hits += 1
	if splash_hits > 0:
		_show_message("Cleave hit x%d" % splash_hits, 0.45)
	if GameManager:
		...
		if milestone_idx >= 0 and milestone_idx < KILL_MILESTONES.size():
			...
			if total_kills >= target:
				...
				_mine_journal_event("Combat milestone %d kills reached (+%dg)." % [target, milestone_gold])
				_show_message("Milestone reached: %d kills!" % target, 1.3)
	if _kill_streak > 0 and _kill_streak % KILL_STREAK_STEP == 0 and GameManager:
		...
		_show_message("%s streak %d! +%dg" % [tier, _kill_streak, bonus_gold], 1.1)
		_mine_journal_event("Combat bonus: %s streak %d (+%dg)." % [tier, _kill_streak, bonus_gold])
		if _kill_streak >= 10 and not bool(GameManager.player_data.get("combat_badge_streak_10", false)):
			...
			_mine_journal_event("Badge unlocked: Streak x10.")
			_show_message("Badge: Streak x10", 1.2)
	if GatheringSfx:
		GatheringSfx.play_mine_swing()
	_play_screen_shake(3.2)
	_mine_journal_event("Defeated %s." % enemy.enemy_id)
	_mine_ui_refresh()
		...
		for o in objectives:
			...
			if goal > 0 and cur < goal and float(cur) / float(goal) >= 0.8:
				if not bool(_quest_near_done_latched.get(str(qid), false)):
					...
					_show_message("Almost done: %s" % str(q.get("title", qid)), 0.65)
...
func _try_award_streak_medal(streak: int) -> void:
	...
	_show_message("%s unlocked! +%dg" % [str(d.get("name", "Medal")), bonus], 1.0)
	_mine_journal_event("Streak medal unlocked: %s (+%dg)." % [str(d.get("name", "Medal")), bonus])
