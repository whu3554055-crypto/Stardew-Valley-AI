...
5. **清单与实现一致**：凡在本文件标为 `[x]` 的条目，须在工程内具备**可运行路径**（非仅文档表述）；发现脱节时以代码修复或改回 `[ ]` 为准。  
   - **验收清单（可跳转目录 + 批处理）**：[docs/03-研发管理/11-TODO已实现项验收清单.md](docs/03-研发管理/11-TODO已实现项验收清单.md)；一键自动化：`.\tools\run_todo_acceptance.ps1`（含与 `run_gut.ps1` 相同的**全量** `tests/unit` GUT；可选 `-SkipGut` 仅冒烟）。  
6. **玩家可见后果（反暗箱，长期遵守）**  
