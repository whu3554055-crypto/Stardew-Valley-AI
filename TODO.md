...
- [ ] **A3 — 呈现与资源**  
  ...
  *进度：`TileMap` 已挂 `GameTileMap` + 运行时/资源图集 `assets/tiles/terrain_atlas_32.png`；玩家已用 `player.png`（缩放至约 32px）。田间作物按阶段 `Sprite2D`；背包格对有图标的物品显示缩略图 + `ItemDatabase.resolve_icon_path`。犁地会同步底层瓦片为犁土贴图；种子/部分工具用别名或占位图。存档：`user://farm.save`、`user://inventory.save`、`user://quests.save` 与 `GameManager` 同步读档；图鉴 `meals` 已写入 `gathering_almanac.save`；旧档无 inventory 时补发一次新手包。*
