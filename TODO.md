...
- [ ] **A1 — 单系统加深（第一轮）**  
  ...
  *进度：已加深钓鱼 / 挖矿 / 烹饪各一轮（新鱼种与垃圾、任务只计真鱼、矿层石英与铁镐提示、新食谱与图鉴料理统计）；可继续调数值与指引。*

- [ ] **A2 — 单系统加深（第二轮）**  
  换另一条系统重复 A1 的标准，避免长期只堆一条线。  
  *进度：钓鱼新增鲶鱼/鲭鱼与表修正；挖矿矿层前缀（表层/铁带/深脉）、主题权重、表层 geode；烹饪四条三料复合食谱。*

- [ ] **A3 — 呈现与资源**  
  补齐或替换占位：**TileMap tileset、玩家/作物/工具相关美术**，让玩法反馈「看得见」（可与 A1/A2 并行，由人力安排）。  
  *进度：`TileMap` 已挂 `GameTileMap` + 运行时/资源图集 `assets/tiles/terrain_atlas_32.png`；玩家已用 `player.png`（缩放至约 32px）。田间作物按阶段 `Sprite2D`；背包格对有图标的物品显示缩略图 + `ItemDatabase.resolve_icon_path`。犁地会同步底层瓦片为犁土贴图；种子/部分工具用别名或占位图。存档：主存档为 **`user://game_save.bundle`**（玩家/农场/背包/任务/简报/图鉴）；无 bundle 时回退旧多文件 + `gathering_almanac.load_data`；图鉴仍写 `gathering_almanac.save`；旧档无 inventory 时补发一次新手包。*
